...
| 工具 | 版本要求 | 说明 | 安装检查 |
|------|---------|------|---------|
| **Node.js** | 20.19+ | 前端运行环境，包含 npm | `node -v` |
...
默认会读取根目录下的 `.env`，并映射端口 `5001`。生产容器由后端通过 Waitress 同时提供 API 与前端静态文件，访问地址为 `http://localhost:5001`。

上传、任务与报告数据会持久化到 `./backend/uploads`。
